...
This Node.js service sends an optional Blobtown `_bt` blob/file ID to a Discord webhook and attaches every request header in `request-headers.txt`, then redirects the browser to the matching Blobtown inventory URL. Sensitive header values are replaced by `[redacted]`.
...
The webhook URL stays server-side. Every received header name is included in the attachment, while authentication, cookie, token, key, session, credential, signature, referrer, and client-IP values are redacted. The redirect destination is fixed to Blobtown, mention parsing is disabled, `_bt` is limited to 1,000 characters, and requests are rate-limited per client IP.

## Customize it

Open `config.js` to edit the header blacklist, blob-ID limit, rate limit, or fixed Blobtown destination. Add full header names to `blockedHeaderNames`, or add reusable fragments such as `token` to `blockedHeaderTerms`. Do not remove the authentication, cookie, or credential protections.
